                    ...
                    switch (MenuProvider.Orbwalker.ActiveMode)
                    {
                                ...
                                if (_e.IsReadyPerfectly())
                                {
                                    var target =
                                        HeroManager.Enemies.OrderByDescending(x => TargetSelector.GetPriority(x))
                                            .FirstOrDefault(
                                                x => x.LSIsValidTarget(_e.Range + Orbwalking.GetRealAutoAttackRange(x)));
                                    if (target != null)
                        ...
                        case Orbwalking.OrbwalkingMode.LaneClear:
                        {
                            if (MenuProvider.Champion.Laneclear.UseW)
                                if (!ObjectManager.Player.LSIsDashing())
                                    if (_w.IsReadyPerfectly())
                                        if (MinionManager.GetMinions(_w.Range).Count(x => x.LSIsValidTarget(_w.Range)) >=
                                            3)
                                            _w.Cast();
                                    ...
                                    if (_w.IsReadyPerfectly())
                                        if (
                                            MinionManager.GetMinions(_w.Range, MinionTypes.All, MinionTeam.Neutral,
                                                MinionOrderTypes.MaxHealth).Any(x => x.LSIsValidTarget(_w.Range)))
                                            _w.Cast();
                            break;
                        }
                    }
