...
namespace Velkoz
{
    internal class Program
    {
        ...
        private static void UseSpells(bool useQ, bool useW, bool useE, bool useR, bool useIgnite)
        {
            ...
            if (useQ && qTarget != null && Q.LSIsReady() && Q.Instance.ToggleState == 0)
            {
                if (Q.Cast(qTarget) == Spell.CastStates.SuccessfullyCasted)
                    return;
            }

            if (qDummyTarget != null && useQ && Q.LSIsReady() && Q.Instance.ToggleState == 0)
            {
        ...
        private static void Farm()
        {
            ...
            if (useQ && allMinionsW.Count > 0 && Q.Instance.ToggleState == 0 && Q.LSIsReady())
            {
        ...
        private static void JungleFarm()
        {
            ...
            {
                var mob = mobs[0];
                if (useQ && Q.Instance.ToggleState == 0 && Q.LSIsReady())
                    Q.Cast(mob);
