...
namespace Karma
{
    internal class Program
    {
        ...
        {
            if (sender.LSIsValidTarget(1000f) && args.DangerLevel == Interrupter2.DangerLevel.High && _e.LSIsReady())
            {
                _r.Cast();
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (menuItem.Active)
            {
                foreach (var enemy in
                    ObjectManager.Get<AIHeroClient>().Where(h => h.LSIsValidTarget() && h.HasBuff("KarmaSpiritBind")))
                {
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            if (_config.Item("UseEDefense").GetValue<bool>())
            {
                foreach (var hero in
                    ObjectManager.Get<AIHeroClient>()
                        .Where(
                            hero =>
                                hero.LSIsValidTarget(_e.Range, false) && hero.IsAlly &&
            ...
            if (wActive && wTarget != null && _w.LSIsReady())
            {
            ...
            if (qActive && qTarget != null && _q.LSIsReady())
            {
                if (rActive)
                {
                    _r.Cast();
