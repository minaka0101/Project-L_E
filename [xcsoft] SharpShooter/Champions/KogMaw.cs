...
namespace Sharpshooter.Champions
{
    public static class KogMaw
    {
       ...
       // R 2 = 1500
        ...
        {
            return R.LSIsReady() ? R.GetDamage(enemy) : 0;
        }
        ...
        static void Combo()
        {
            ...
            if (SharpShooter.Menu.Item("comboUseQ", true).GetValue<Boolean>() && Q.LSIsReady())
            {
            ...
            if (SharpShooter.Menu.Item("comboUseW", true).GetValue<Boolean>() && W.LSIsReady())
            {
                ...
                if (W.LSIsReady() && Wtarget.LSIsValidTarget(GetWActiveRange))
                    W.Cast();
            }
            ...
            if (SharpShooter.Menu.Item("comboUseR", true).GetValue<Boolean>() && R.LSIsReady())
            {
                ...
                if (ConfigMenu.SelectedPrediction.SelectedIndex == 0)
                {
                    ...
                    if (R.LSIsReady() && Rtarget.LSIsValidTarget(GetRRange) && Player.ServerPosition.LSTo2D().LSDistance(Rpred.CastPosition, false) < GetRRange && Rpred.HitChance >= HitChance.High)
                        R.Cast(Rpred.CastPosition);
                }
                else
                {
                    var Rpred = R.GetPrediction(Rtarget);
                    if (R.LSIsReady() && Rtarget.LSIsValidTarget(GetRRange) && Player.ServerPosition.LSDistance(Rpred.CastPosition, false) < GetRRange && Rpred.Hitchance >= HitChance.High)
                        R.Cast(Rpred.CastPosition);
                }
            }
        }
        ...
        static void Harass()
        {
            ...
            if (SharpShooter.Menu.Item("harassUseE", true).GetValue<Boolean>() && E.LSIsReady())
            {
                var Etarget = TargetSelector.GetTarget(Q.Range, TargetSelector.DamageType.Magical, true);
            ...
            if (SharpShooter.Menu.Item("harassUseR", true).GetValue<Boolean>() && R.LSIsReady())
            {
                var Rtarget = TargetSelector.GetTarget(GetRRange, TargetSelector.DamageType.Magical, true);
                var Rpred = R.GetPrediction(Rtarget);

                if (R.LSIsReady() && Rtarget.LSIsValidTarget(GetRRange) && Player.ServerPosition.LSDistance(Rpred.CastPosition, false) < GetRRange && Rpred.Hitchance >= HitChance.VeryHigh)
                {
                    R.Cast(Rpred.CastPosition);
                    return;
                }
        ...
        static void Laneclear()
        {
            ...
            if (SharpShooter.Menu.Item("laneclearUseW", true).GetValue<Boolean>() && W.LSIsReady())
            {
                if (Minions.Where(x => x.LSIsValidTarget(GetWActiveRange + 100)).Count() >= 3)
                    W.Cast();
            }
            ...
            if (SharpShooter.Menu.Item("laneclearUseE", true).GetValue<Boolean>() && E.LSIsReady())
            {
                var Eloc = E.GetLineFarmLocation(Minions.Where(x => x.LSIsValidTarget(E.Range)).ToList());
            ...
            if (SharpShooter.Menu.Item("laneclearUseR", true).GetValue<Boolean>() && R.LSIsReady())
            {
                var Rloc = R.GetCircularFarmLocation(Minions.Where(x => x.LSIsValidTarget(GetRRange)).ToList());
        ...
        static void Jungleclear()
        {
            ...
            if (SharpShooter.Menu.Item("jungleclearUseW", true).GetValue<Boolean>() && W.LSIsReady())
            {
                W.Cast();
                return;
            }
            ...
            if (SharpShooter.Menu.Item("jungleclearUseE", true).GetValue<Boolean>() && E.LSIsReady())
            {
                E.Cast(Mobs[0]);
                return;
            }

            if (SharpShooter.Menu.Item("jungleclearUseR", true).GetValue<Boolean>() && R.LSIsReady())
            {
                R.Cast(Mobs[0]);
                return;
            }
