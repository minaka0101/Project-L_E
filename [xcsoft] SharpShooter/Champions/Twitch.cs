...
namespace Sharpshooter.Champions
{
    public static class Twitch
    {
        ...
        static void Game_OnUpdate(EventArgs args)
        {
            ...
            {
                if (Q.LSIsReady() && Recall.LSIsReady())
                {
                    Q.Cast();
                    Recall.Cast();
                }
            ...
            if (SharpShooter.Menu.Item("drawingRLine", true).GetValue<Boolean>())
            {
                if (Player.HasBuff("TwitchFullAutomatic"))
                {
                    ...
                    if(aatarget != null)
                    {
                        ...
                        var dis = (Orbwalking.GetRealAutoAttackRange(Player) + 300) - Player.LSDistance(aatarget, false);

                        var to = Drawing.WorldToScreen(dis > 0 ? aatarget.ServerPosition.LSExtend(Player.Position, -dis) : aatarget.ServerPosition);
        ...
        static void Killsteal()
        {
            ...
            foreach (AIHeroClient target in HeroManager.Enemies.Where(x => x.LSIsValidTarget(E.Range) && !x.HasBuffOfType(BuffType.Invulnerability) && !x.HasBuffOfType(BuffType.SpellShield)))
            {
                if (target != null)
                {
        ...
        {
            return E.LSIsReady() ? E.GetDamage(enemy) : 0;
        }
        ...
        static void Laneclear()
        {
            ...
            if (E.LSIsReady() && SharpShooter.Menu.Item("laneclearUseE", true).GetValue<Boolean>())
            {
                var killcount = 0;
