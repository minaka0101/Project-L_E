...
namespace Sharpshooter.Champions
{
    public static class Teemo
    {
        ...
        static void Combo()
        {
            ...
            {
                var Wtarget = HeroManager.Enemies.Where(x => x.LSIsValidTarget(1000) && !x.LSIsFacing(Player)).FirstOrDefault();
            ...
            {
                var Rtarget = HeroManager.Enemies.Where(x => R.CanCast(x) && !x.HasBuff("bantamtraptarget")).OrderBy(x => x.LSDistance(Player.Position, false)).FirstOrDefault();
        ...
        {
            var bigminion = MinionManager.GetMinions(Q.Range, MinionTypes.All, MinionTeam.Enemy).Where(x => Q.CanCast(x) && x.Health <= Q.GetDamage(x) && (x.BaseSkinName.ToLower().Contains("siege") || x.BaseSkinName.ToLower().Contains("super"))).FirstOrDefault();
