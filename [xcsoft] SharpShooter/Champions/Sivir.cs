...
namespace Sharpshooter.Champions
{
    public static class Sivir
    {
        ...
        static void AIHeroClient_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (sender is AIHeroClient && sender.IsEnemy && args.Target.IsMe && !args.SData.LSIsAutoAttack() && E.LSIsReady())
                E.Cast();
        }

        static void Orbwalking_OnAfterAttack(AttackableUnit unit, AttackableUnit target)
        {
            if (!W.LSIsReady() || !unit.IsMe || !(target.Type == GameObjectType.AIHeroClient))
                return;
        ...
        static void AutoQ()
        {
            ...
            foreach (AIHeroClient target in HeroManager.Enemies.Where(x => x.LSIsValidTarget(Q.Range) && !x.HasBuffOfType(BuffType.Invulnerability)))
            {
                if (target != null)
                {
