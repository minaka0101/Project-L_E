...
namespace Sharpshooter.Champions
{
    public static class Ezreal
    {
        ...
        static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (SharpShooter.Menu.Item("harassAuto", true).GetValue<KeyBind>().Active && !Player.LSUnderTurret(true) && Orbwalker.ActiveMode != Orbwalking.OrbwalkingMode.Combo)
                Harass();
        ...
        static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Q.LSIsReady() && drawingQ.Active)
        ...
        static void Killsteal()
        {
            ...
            foreach (AIHeroClient target in HeroManager.Enemies.Where(x => x.LSIsValidTarget(R.Range) && !x.HasBuffOfType(BuffType.Invulnerability) && !x.HasBuffOfType(BuffType.SpellShield)))
            {
                ...
                if (R.CanCast(target) && target.Health + (target.HPRegenRate/2) <= R.GetDamage(target) && !target.LSIsValidTarget(Orbwalking.GetRealAutoAttackRange(Player)) && R.GetPrediction(target).Hitchance >= HitChance.VeryHigh)
                    R.Cast(target);
            }
        }
        ...
        static void Orbwalking_AfterAttack(AttackableUnit unit, AttackableUnit target)
        {
            ...
            if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
            {
                if (Q.CanCast(Target) && SharpShooter.Menu.Item("comboUseQ", true).GetValue<Boolean>())
                    Q.SPredictionCast(Target as AIHeroClient, Q.MinHitChance);
                else if (W.CanCast(Target) && SharpShooter.Menu.Item("comboUseW", true).GetValue<Boolean>())
                    W.SPredictionCast(Target as AIHeroClient, W.MinHitChance);
            }
        ...
        static Boolean ExtraCheckForFarm(Obj_AI_Base minion)
        {
            ...
            if (minion.LSIsValidTarget(Orbwalking.GetRealAutoAttackRange(Player) + minion.BoundingRadius))
            {
                if (minion.Health <= Player.LSGetAutoAttackDamage(minion, true))
                    return false;
                else
                    return true;
            }
        ...
        static void Combo()
        {
            ...
            if (SharpShooter.Menu.Item("comboUseR", true).GetValue<Boolean>())
            {
                ...
                {
                    if (R.CanCast(Rtarget) && !Rtarget.LSIsValidTarget(Orbwalking.GetRealAutoAttackRange(Player)))
        ...
        static void Harass()
        {
            if (!Orbwalking.CanMove(1) || !(SharpShooter.getManaPer > SharpShooter.Menu.Item("harassMana", true).GetValue<Slider>().Value) || Player.LSIsRecalling())
                return;
