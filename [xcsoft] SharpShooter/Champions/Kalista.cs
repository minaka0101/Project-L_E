...
namespace Sharpshooter.Champions
{
    public static class Kalista
    {
        ...
        static void AIHeroClient_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (SharpShooter.Menu.Item("soulboundsaver", true).GetValue<Boolean>() && R.LSIsReady())
            {
        ...
        {
            if (!SharpShooter.Menu.Item("killsteal", true).GetValue<Boolean>() || !E.LSIsReady())
                return;
        ...
        {
            if (!SharpShooter.Menu.Item("mobsteal", true).GetValue<Boolean>() || !E.LSIsReady())
                return;
        ...
        static List<Obj_AI_Base> Q_GetCollisionMinions(AIHeroClient source, Vector3 targetposition)
        {
            ...
            return Collision.GetCollision(new List<Vector3> { targetposition }, input).OrderBy(obj => obj.LSDistance(source, false)).ToList();
        }
        ...
        static void Combo()
        {
            ...
            {
                var eTarget = HeroManager.Enemies.Where(x => x.LSIsValidTarget(E.Range) && E.GetDamage(x) >= 1 && !x.HasBuffOfType(BuffType.Invulnerability) && !x.HasBuffOfType(BuffType.SpellShield)).OrderByDescending(x => E.GetDamage(x)).FirstOrDefault();
        ...
        static void Laneclear()
        {
            ...
            if (SharpShooter.Menu.Item("laneclearUseQ", true).GetValue<Boolean>() && Q.LSIsReady())
            {
                //-------------------------------------------------------------------------------------------------------------------------------
                foreach (var minion in Minions.Where(x => x.Health <= Q.GetDamage(x)))
                {
                    ...
                    foreach (var colminion in Q_GetCollisionMinions(Player, Player.ServerPosition.LSExtend(minion.ServerPosition, Q.Range)))
                    {
