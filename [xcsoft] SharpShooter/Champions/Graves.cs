...
namespace Sharpshooter.Champions
{
    public static class Graves
    {
        ...
        static void Orbwalking_AfterAttack(AttackableUnit unit, AttackableUnit target)
        {
            ...
            if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
            {
                if (SharpShooter.Menu.Item("comboUseQ", true).GetValue<Boolean>() && Q.CanCast(Target) && !Player.LSIsDashing())
                    Q.Cast(Target);
            ...
            {
                if (SharpShooter.Menu.Item("harassUseQ", true).GetValue<Boolean>() && Q.CanCast(Target) && !Player.LSIsDashing())
                    Q.Cast(Target);
            }
        }
        ...
        static void Combo()
        {
            ...
            if (SharpShooter.Menu.Item("comboUseQ", true).GetValue<Boolean>() && Q.LSIsReady())
            {
                ...
                if (Q.CanCast(Qtarget) && !Player.LSIsDashing())
                    Q.Cast(Qtarget);
            }
        ...
        static void Killsteal()
        {
            ...
            {
                if (Q.LSIsReady() && target.LSIsValidTarget(850f) && Q.IsKillable(target))
                    Q.Cast(target);
