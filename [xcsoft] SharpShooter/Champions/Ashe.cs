...
namespace Sharpshooter.Champions
{
    public static class Ashe
    {
        ...
        static void Interrupter2_OnInterruptableTarget(AIHeroClient sender, Interrupter2.InterruptableTargetEventArgs args)
        {
            ...
            if (sender.LSIsValidTarget(1500))
            {
                if (R.CanCast(sender))
                    R.Cast(sender);
        ...
        static void AutoR()
        {
            ...
            foreach (AIHeroClient target in HeroManager.Enemies.Where(x => x.LSIsValidTarget(R.Range)))
            {
                if (R.CanCast(target) && UnitIsImmobileUntil(target) >= R.Delay + (Player.LSDistance(target, false) / R.Speed) && CollisionCheck(Player, target, R.Width))
                    R.Cast(target);
            }
        }
        ...
        static void Combo()
        {
            ...
            if (SharpShooter.Menu.Item("comboUseR", true).GetValue<Boolean>())
            {
                ...
                if (R.LSIsReady() && Rtarget.LSIsValidTarget(Orbwalking.GetRealAutoAttackRange(Player) + 50) && !Rtarget.HasBuffOfType(BuffType.SpellImmunity) && R.GetPrediction(Rtarget).Hitchance >= HitChance.VeryHigh && CollisionCheck(Player, Rtarget, R.Width))
                    R.Cast(Rtarget);
            }
        ...
        static void Laneclear()
        {
            ...
            if (W.LSIsReady() && SharpShooter.Menu.Item("laneclearUseW", true).GetValue<Boolean>())
            {
        ...
        static void Jungleclear()
        {
            ...
            if (W.LSIsReady() && SharpShooter.Menu.Item("jungleclearUseW", true).GetValue<Boolean>())
