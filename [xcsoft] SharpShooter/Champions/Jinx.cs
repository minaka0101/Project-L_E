...
namespace Sharpshooter.Champions
{
    public static class Jinx
    {
        ...
        static bool QisActive { get { return Player.HasBuff("JinxQ"); } }
            ...
            if (SharpShooter.Menu.Item("drawingPTimer", true).GetValue<Boolean>())
            {
                foreach (var buff in Player.Buffs)
                {
                    if (buff.Name == "jinxpassivekill")
                    {
                        ...
                        Drawing.DrawText(targetpos[0] - 10, targetpos[1], Color.Gold, "" + (buff.EndTime - Game.Time));
                        break;
                    }
                }
            }
        ...
        static void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            if (E.LSIsReady() && Player.LSDistance(gapcloser.End, false) <= 200)
        ...
        static void QSwitchForUnit(AttackableUnit Unit)
        {
            ...
            if (Utility.LSCountEnemiesInRange(Unit.Position, 200) >= SharpShooter.Menu.Item("FISHBONES", true).GetValue<Slider>().Value)
            {
                QSwitch(true);
                return;
            }

            if (Unit.LSIsValidTarget(DefaultRange))
                QSwitch(false);
            else
                QSwitch(true);
        ...
        static void QSwitch(Boolean activate)
        {
            ...
            if (QisActive && Player.Spellbook.IsAutoAttacking)
                return;
        ...
        static void AutoE()
        {
            ...
            foreach (AIHeroClient target in HeroManager.Enemies.Where(x => x.LSIsValidTarget(E.Range)))
            {
        ...
        static bool CollisionCheck(AIHeroClient source, AIHeroClient target, float width)
        {
        ...
        {
            return ObjectManager.Get<Obj_AI_Minion>().Count(h => h.LSIsValidTarget(range, true, point));
        }
        ...
        static float GetComboDamage(Obj_AI_Base enemy)
        {
            ...
            if (R.LSIsReady())
                damage += (float)(new double[] { 25, 30, 35 }[R.Level] / 100 * (enemy.MaxHealth - enemy.Health) + ((new double[] { 25, 35, 45 }[R.Level] + 0.1 * Player.FlatPhysicalDamageMod) * Math.Min((1 + Player.LSDistance(enemy.ServerPosition) / 15 * 0.09d), 10)));
        ...
        {
            return HeroManager.Enemies.Where(x => E.CanCast(x) && !x.HasBuffOfType(BuffType.SpellImmunity) && E.GetPrediction(x).Hitchance >= HitChance.VeryHigh && !x.LSIsFacing(Player) && x.IsMoving && x.LSIsValidTarget(DefaultRange)).OrderBy(x => x.LSDistance(Player, false)).FirstOrDefault();
        }
        ...
        static void Combo()
        {
            ...
            if (SharpShooter.Menu.Item("comboUseW", true).GetValue<Boolean>())
            {
                ...
                if (W.CanCast(Wtarget) && !Wtarget.LSIsValidTarget(DefaultRange / 3))
                    W.SPredictionCast(Wtarget, W.MinHitChance);
            }

            if (SharpShooter.Menu.Item("comboUseE", true).GetValue<Boolean>() && E.LSIsReady())
            {
            ...
            if (SharpShooter.Menu.Item("comboUseR", true).GetValue<Boolean>() && R.LSIsReady() && WLastCastedTime + 1.0 < Game.Time && !Player.Spellbook.IsAutoAttacking)
            {
                foreach (AIHeroClient Rtarget in HeroManager.Enemies.Where(x => CollisionCheck(Player, x, R.Width) && x.LSIsValidTarget(R.Range) && !x.LSIsValidTarget(Orbwalking.GetRealAutoAttackRange(Player)) && !Player.HasBuffOfType(BuffType.SpellShield) && !Player.HasBuffOfType(BuffType.Invulnerability) && R.GetPrediction(x).Hitchance >= HitChance.High && Utility.LSGetAlliesInRange(x, 800).Where(ally => !ally.IsMe).Count() <= 1))
                {
                    var dis = Player.LSDistance(Rtarget.ServerPosition);
                    double predhealth = HealthPrediction.GetHealthPrediction(Rtarget, (int)(R.Delay + dis / R.Speed) * 1000) + Rtarget.HPRegenRate;

                    var RDamage = (new double[] { 25, 30, 35 }[R.Level] / 100 * (Rtarget.MaxHealth - Rtarget.Health) + ((new double[] { 25, 35, 45 }[R.Level] + 0.1 * Player.FlatPhysicalDamageMod) * Math.Min((1 + Player.LSDistance(Rtarget.ServerPosition) / 15 * 0.09d), 10)));
        ...
        static void Harass()
        {
            ...
            if (SharpShooter.Menu.Item("harassUseQ", true).GetValue<Boolean>() && Q.LSIsReady())
                QSwitchForUnit(TargetSelector.GetTarget(GetQActiveRange + 30, TargetSelector.DamageType.Physical, true));

            if (SharpShooter.Menu.Item("harassUseW", true).GetValue<Boolean>() && W.LSIsReady())
            {
                var Wtarget = TargetSelector.GetTarget(W.Range, TargetSelector.DamageType.Physical, true);
        ...
        static void Jungleclear()
        {
            ...
            if (SharpShooter.Menu.Item("jungleclearUseQ", true).GetValue<Boolean>() && Q.LSIsReady())
            {
                var target = Orbwalker.GetTarget();
