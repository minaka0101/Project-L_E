    ...
    public static class Vayne
    {
                ...
                if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
                    if (SharpShooter.Menu.Item("QMode", true).GetValue<StringList>().SelectedIndex == 0)
                        if (SharpShooter.Menu.Item("comboUseQ", true).GetValue<Boolean>())
                            if (!Player.Spellbook.IsAutoAttacking && Q.LSIsReady())
        ...
        static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (drawingCond.Active)
                foreach (var En in HeroManager.Enemies.Where(hero => hero.LSIsValidTarget(1100)))
                {
                    ...
                    {
                        Vector3 loc3 = EPred.UnitPosition.LSTo2D().LSExtend(Player.Position.LSTo2D(), -i).To3D();
        ...
        static void AIHeroClient_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (SharpShooter.Menu.Item("AutoRQ", true).GetValue<Boolean>())
                if (sender.IsMe && args.SData.Name == Player.Spellbook.GetSpell(SpellSlot.R).SData.Name && Q.LSIsReady())
                    Q.Cast(Game.CursorPos);
        }
        ...
        static float GetComboDamage(Obj_AI_Base enemy)
        {
            foreach (var Buff in enemy.Buffs)
                if (Buff.Name == "vaynesilvereddebuff" && Buff.Count == 2)
                    return W.GetDamage(enemy) + (float)Player.LSGetAutoAttackDamage(enemy, true);
        ...
        static void LogicalQ(Boolean combomode)
        {
            ...
            var QEndpos = Player.ServerPosition.LSExtend(Game.CursorPos, 300);

            foreach (var enemy in QEndpos.LSGetEnemiesInRange(308))
                if (enemy.IsMelee())
                    return;

            if (!combomode)
                if (QEndpos.LSUnderTurret(true) || QEndpos.LSCountEnemiesInRange(800) > 0)
                    return;
        ...
        static void Combo()
        {
            ...
            if (SharpShooter.Menu.Item("comboUseQ", true).GetValue<Boolean>() && SharpShooter.Menu.Item("QMode", true).GetValue<StringList>().SelectedIndex == 1 && !Player.Spellbook.IsAutoAttacking)
            {
            ...
            if (E.LSIsReady() && SharpShooter.Menu.Item("comboUseE", true).GetValue<Boolean>())
                foreach (var En in HeroManager.Enemies.Where(hero => hero.LSIsValidTarget(E.Range) && !hero.HasBuffOfType(BuffType.SpellShield) && !hero.HasBuffOfType(BuffType.SpellImmunity)))
                {
                    //Part of VayneHunterRework

                    var EPred = E.GetPrediction(En);
                    int pushDist = 425;
                    var FinalPosition = EPred.UnitPosition.LSTo2D().LSExtend(Player.ServerPosition.LSTo2D(), -pushDist).To3D();

                    for (int i = 1; i < pushDist; i += (int)En.BoundingRadius)
                    {
                        Vector3 loc3 = EPred.UnitPosition.LSTo2D().LSExtend(Player.ServerPosition.LSTo2D(), -i).To3D();

                        if (loc3.LSIsWall() || isAllyFountain(FinalPosition))
                            E.Cast(En);
                    }
                }
        ...
        static void Harass()
        {
            ...
            if (Q.CanCast(target) && SharpShooter.Menu.Item("harassUseQ", true).GetValue<Boolean>() && !Player.Spellbook.IsAutoAttacking)
                LogicalQ(true);
        ...
        static void Laneclear()
        {
            ...
            var Minions = MinionManager.GetMinions(Player.ServerPosition.LSExtend(Game.CursorPos, 300), Orbwalking.GetRealAutoAttackRange(Player), MinionTypes.All, MinionTeam.Enemy);
