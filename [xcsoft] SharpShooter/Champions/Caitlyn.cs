    ...
    public static class Caitlyn
    {
        ...
        static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (SharpShooter.Menu.Item("dash", true).GetValue<KeyBind>().Active)
            {
                if(E.LSIsReady())
                {
                    E.Cast(Game.CursorPos.LSExtend(Player.Position, 5000));
        ...
        static void AutoW()
        {
            ...
            foreach (AIHeroClient target in HeroManager.Enemies.Where(x => x.LSIsValidTarget(W.Range)))
            {
                if (target != null)
                {
        ...
        {
            return HeroManager.Enemies.Where(x => W.CanCast(x) && !x.HasBuffOfType(BuffType.SpellImmunity) && W.GetPrediction(x).Hitchance >= HitChance.VeryHigh && !x.LSIsFacing(Player) && x.LSIsValidTarget(550)).OrderBy(x => x.LSDistance(Player, false)).FirstOrDefault();
        }
        ...
        static void Combo()
        {
            ...
            {
                foreach (var Rtarget in HeroManager.Enemies.Where(t => t.LSIsValidTarget(1500 + (500 * R.Level)) && !t.LSIsValidTarget(Orbwalking.GetRealAutoAttackRange(Player)) && !t.HasBuffOfType(BuffType.Invulnerability) && !t.HasBuffOfType(BuffType.SpellShield)))
                {
