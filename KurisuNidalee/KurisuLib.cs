...
namespace KurisuNidalee
{
    internal static class KurisuLib
    {
        ...
        internal static AIHeroClient _Player = ObjectManager.Player;
        ...
        internal static void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            if (attacker.LSIsValidTarget(275f) && !_Player.LSUnderTurret(true))
            {
                if (CatForm())
                {
        ...
        internal static bool CanUse(Spell spell, bool human, string mode)
        {
            ...
            var learned = !(spell.Level <= 0 || _Player.Spellbook.GetSpell(spell.Slot).State == SpellState.NotLearned);
        ...
        {
            return _Player.CharData.BaseSkinName != "Nidalee";
        }
        ...
        internal static float CatDamage(Obj_AI_Base target)
        {
            ...
            if (SpellTimer["Takedown"].IsReady())
                damage += _Player.LSGetSpellDamage(target, SpellSlot.Q, 1);
            if (SpellTimer["Pounce"].IsReady())
                damage += _Player.LSGetSpellDamage(target, SpellSlot.W, 1);
            if (SpellTimer["Swipe"].IsReady())
                damage += _Player.LSGetSpellDamage(target, SpellSlot.E, 1);

            return (float) (damage + _Player.LSGetAutoAttackDamage(target) * 2);
        }
        ...
        {
            var minionpositions =
                MinionManager.GetMinions(_Player.ServerPosition, 500f, MinionTypes.All, MinionTeam.NotAllyForEnemy)
        ...
        private static void HeroOnCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            try
            {
                if (KN.Root.Item("ndhwsp").GetValue<bool>())
                {
                    ...
                    {
                        if (args.End.LSIsValid() && args.End.LSDistance(KN._Player.ServerPosition) <= KN._Player.BoundingRadius * 2)
                        {
                ...
                if (sender.IsMe && args.SData.Name.ToLower() == "pounce")
                {
                    ...
                    if (unit.IsValid<Obj_AI_Base>() && unit.IsHunted())
                        ...
                        TimeStamp["Pounce"] = Game.Time + (5 + (5 * _Player.PercentCooldownMod));
                }

                if (sender.IsMe && args.SData.Name.ToLower() == "swipe")
                    TimeStamp["Swipe"] = Game.Time + (5 + (5 * _Player.PercentCooldownMod));

                if (sender.IsMe && args.SData.Name.ToLower() == "primalsurge")
                    TimeStamp["Primalsurge"] = Game.Time + (12 + (12 * _Player.PercentCooldownMod));

                if (sender.IsMe && args.SData.Name.ToLower() == "bushwhack")
                {
                    var wperlevel = new[] { 13, 12, 11, 10, 9 }[Spells["Bushwhack"].Level - 1];
                    TimeStamp["Bushwhack"] = Game.Time + (wperlevel + (wperlevel * _Player.PercentCooldownMod));
                }

                if (sender.IsMe && args.SData.Name.ToLower() == "javelintoss")
                {
                    Counter = 0;
                    TimeStamp["Javelin"] = Game.Time + (6 + (6 * _Player.PercentCooldownMod));
                }
                ...
                if (sender.IsMe && args.SData.LSIsAutoAttack() && _Player.LSHasBuff("Takedown", true))
                {
                    LastBite = Utils.GameTimeTickCount;
                    TimeStamp["Takedown"] = Game.Time + (5 + (5 * _Player.PercentCooldownMod));
                }
            }
        ...
        internal static void GetSmiteSlot()
        {
            ...
            if (_Player.GetSpell(SpellSlot.Summoner2).Name.ToLower().Contains("smite"))
            {
                SmiteInGame = true;
        ...
        internal static void SmiteOnUpdate(EventArgs args)
        {
            ...
            {
                var damage = _Player.Spellbook.GetSpell(Smite).State == SpellState.Ready
                    ? (float) _Player.GetSummonerSpellDamage(minion, Damage.SummonerSpell.Smite)
                    : 0;

                if (minion.LSDistance(_Player.ServerPosition) > 500 + minion.BoundingRadius + _Player.BoundingRadius)
                    return;
                ...
                if (LargeList.Any(name => minion.Name.StartsWith(name) && !minion.Name.Contains("Mini")))
                {
                    if (KN.Root.Item("jgsmitebg").GetValue<bool>())
                    {
                        ...
                        {
                            if (_Player.LSGetSpellDamage(minion, SpellSlot.Q, 1) + damage >= minion.Health)
                                CM.CastTakedown(minion, "jg");
                        }
                            
                        if (damage >= minion.Health)
                        {
                            _Player.Spellbook.CastSpell(Smite, minion);
                        }
                    }
                }
            ...
            // smite hero blu/red
            if (_Player.GetSpell(Smite).Name.ToLower() == "s5_summonersmiteduel" ||
                _Player.GetSpell(Smite).Name.ToLower() == "s5_summonersmiteplayerganker")
            {
                // KS Smite
                if (_Player.GetSpell(Smite).Name.ToLower() == "s5_summonersmiteplayerganker")
                {
                    foreach (
                        var hero in
                            HeroManager.Enemies.Where(
                                h =>
                                    ...
                                    h.Health <= 20 + 8 * _Player.Level))
                    {
                        _Player.Spellbook.CastSpell(Smite, hero);
                    }
                }
