...
namespace KurisuNidalee
{
    internal class CastManager
    {
        ...
        internal static void CastJavelin(Obj_AI_Base target, string mode)
        {
            ...
            {
                if (mode != "ha" || KL._Player.ManaPercent > 65)
                { 
                    if (target.LSIsValidTarget(KL.Spells["Javelin"].Range))
                    {
                        ...
                        if (target.IsChampion())
                        {
                                ...
                                switch (KN.Root.Item("ppred").GetValue<StringList>().SelectedValue)
                                {
                                    case "OKTW":
                                        var pi = new SebbyLib.Prediction.PredictionInput
                                        {
                                            ...
                                            Delay = 0.25f,
                                            Range = 1500f,
                                            From = KN._Player.ServerPosition,
                                            Radius = 40f,
                                            Unit = target,
                            ...
                            if (qoutput.Hitchance == HitChance.Collision && KL.Smite.LSIsReady())
                            {
                                ...
                                {
                                    if (qoutput.CollisionObjects.All(i => i.NetworkId != KL._Player.NetworkId))
                                    {
                                        ...
                                        if (obj.Count == 1)
                                        {
                                            if (obj.Any(
                                                i =>
                                                    i.Health <= KL._Player.GetSummonerSpellDamage(i, Damage.SummonerSpell.Smite) &&
                                                    KL._Player.LSDistance(i) < 500 && KL._Player.Spellbook.CastSpell(KL.Smite, obj.First())))
                                            {
        ...
        {           
            ...
            {
                if (KL._Player.ManaPercent <= 65 && target.IsHunted() && target.CanMove)
                {
                    return;
                }
                ...
                if (mode != "ha" || KL._Player.ManaPercent > 65)
                {
                    if (target.LSIsValidTarget(KL.Spells["Bushwhack"].Range))
                    {
                        ...
                        if (KN.Root.Item("ndhwforce").GetValue<StringList>().SelectedIndex == 1)
                        {
                            ...
                            KL.Spells["Bushwhack"].Cast(unitpos.LSExtend(KL._Player.ServerPosition, -75f));
                        }
                    }
                }
            }
        ...
        internal static void CastTakedown(Obj_AI_Base target, string mode)
        {
            ...
            {
                if (target.LSIsValidTarget(KL._Player.AttackRange + KL.Spells["Takedown"].Range))
                {
        ...
        internal static void CastPounce(Obj_AI_Base target, string mode)
        {
            ...
            {
                // get hitbox
                var radius = KL._Player.AttackRange + KL._Player.LSDistance(KL._Player.BBox.Minimum) + 1;
                ...
                if (target.IsHunted() && KN.Root.Item("ndcwhunt").GetValue<bool>() ||
                    ...
                    target.LSDistance(KL._Player.ServerPosition) > radius ||
                    ...
                    !target.IsHunted() && mode == "co" && !KN.Root.Item("ndcwdistco").GetValue<bool>())
                {
                    if (KN.Root.Item("kitejg").GetValue<bool>() && mode == "jg" &&
                        target.LSDistance(Game.CursorPos) > 600 && target.LSDistance(KL._Player.ServerPosition) <= 300)
                    {
            ...
            else
            {
                ...
                if (target.LSDistance(KL._Player.ServerPosition) > KL.Spells["Pounce"].Range)
                    return;
                ...
                if (target.LSDistance(KL._Player.ServerPosition) > radius || 
        ...
        internal static void SwitchForm(Obj_AI_Base target, string mode)
        {
            ...
            if (KL.CatForm() && KL.CanUse(KL.Spells["Aspect"], false, mode))
            {
                ...
                if (KL.CanUse(KL.Spells["Takedown"], true, mode) && KL._Player.HasBuff("Takedown") &&
                    target.LSDistance(KL._Player.ServerPosition) <= KL.Spells["Takedown"].Range + 65f)
                {
                    return;
                }
                ...
                else
                {
                    ...
                    if (!KL.SpellTimer["Pounce"].IsReady(3) && target.LSDistance(KL._Player.ServerPosition) <= 525)
                    {
                        if (target.LSDistance(KL._Player.ServerPosition) > radius)
                        {
                            if (KL._Player.LSGetAutoAttackDamage(target, true) * 3 >= target.Health)
                                KL.Spells["Aspect"].Cast();
                        }
                    }
                }
                ...
                if (mode == "jg")
                {
                        ...
                        KL.CanUse(KL.Spells["Javelin"], true, mode))
                    {
                        if ((!KL.SpellTimer["Pounce"].IsReady(2) || !KL.CanUse(KL.Spells["Pounce"], false, mode)) &&
                            ...
                            !(KL._Player.LSDistance(target.ServerPosition) <= 355) ||
            ...
            if (!KL.CatForm() && KL.CanUse(KL.Spells["Aspect"], true, mode))
            {
                ...
                if (target.IsHunted())
                {
                    ...
                    if (target.LSDistance(KL._Player) > KL.Spells["Takedown"].Range + 50 &&
                ...
                else
                {
                    ...
                    if (target.LSIsValidTarget(KL.Spells["Pounce"].Range + 55))
                    {
                        if (mode != "jg")
                        {
                            ...
                            if (!KL.CanUse(KL.Spells["Javelin"], true, mode) ||
                                ...
                                ((6 + (6 * KL._Player.PercentCooldownMod)) * 1000) >= 500 &&
                    ...
                    if (KN.Target.LSIsValidTarget(KL.Spells["Javelin"].Range) && target.IsChampion())
                    {
                        ...
                        {
                            // check if in pounce range.
                            if (target.LSDistance(KL._Player.ServerPosition) <= KL.Spells["Pounce"].Range + 100f)
                            {
                        ...
                        if (KN.Target.IsHunted() && KN.Target.LSDistance(KL._Player.ServerPosition) > KL.Spells["ExPounce"].Range + 100)
                        {
                            if (target.LSDistance(KL._Player.ServerPosition) <= KL.Spells["Pounce"].Range + 25)
                            {
                                if (KL.Spells["Aspect"].LSIsReady())
                                    KL.Spells["Aspect"].Cast();
                            }
                        ...
                        if (!KL.SpellTimer["Javelin"].IsReady())
                        {
                            if (target.LSDistance(KL._Player.ServerPosition) <= KL.Spells["Pounce"].Range + 125)
                            {
                                KL.Spells["Aspect"].Cast();
                            }
                        }
