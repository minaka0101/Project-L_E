...
namespace KurisuNidalee
{
    internal class KurisuNidalee
    {
        ...
        {
            if (sender.IsEnemy || sender.Type != _Player.Type || !args.SData.LSIsAutoAttack())
            {
                return;
            }
            ...
            foreach (var ally in Allies().Where(hero => !hero.IsMelee))
            {
                ...
                if (args.Target.Type == GameObjectType.AIHeroClient || args.Target.Type == GameObjectType.obj_AI_Turret)
                {
                    ...
                    if (KL.CanUse(KL.Spells["Primalsurge"], true, "on"))
                    {
                        ...
                        {
                            if (!_Player.Spellbook.IsChanneling && !_Player.LSIsRecalling())
                            {
                                if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.None ||
                                    ally.Health / ally.MaxHealth * 100 <= 20 || !KL.CatForm())
                                {
                                    if (_Player.Mana / _Player.MaxMana * 100 <
        ...
        {
            if (_Player.IsDead || !_Player.IsValid)
            {
                return;
            }
            ...
            {
                var pos = Drawing.WorldToScreen(_Player.Position);
            ...
            {
                Render.Circle.DrawCircle(KL._Player.Position, KL.Spells["Javelin"].Range, Color.FromArgb(155, Color.DeepPink), 4);
            }

            if (Root.Item("dz").GetValue<bool>() && KL.CatForm())
            {
                Render.Circle.DrawCircle(KL._Player.Position, KL.Spells["ExPounce"].Range, Color.FromArgb(155, Color.DeepPink), 4);
            }
        }
            ...
            if (KL.CanUse(KL.Spells["Primalsurge"], true, "on"))
            {
                if (!_Player.Spellbook.IsChanneling && !_Player.LSIsRecalling())
                {
                                        ...
                                        Root.Item("zz" + h.ChampionName).GetValue<Slider>().Value))
                    {
                        ...
                        {
                            if (_Player.Mana / _Player.MaxMana * 100 < Root.Item("ndhemana").GetValue<Slider>().Value &&
        ...
        {
            var target = ObjectManager.Get<Obj_AI_Minion>().Where(x => x.LSDistance(_Player.ServerPosition) <= 600 
            ...
            if (Utils.GameTimeTickCount - KL.LastR >= 500 - Game.Ping)
            {
                ...
                {
                    if (!KL.CatForm() && target.LSIsValidTarget(KL.Spells["Bushwhack"].Range) && KL._Player.ManaPercent > 40)
                    {
        ...
        {
            var minions = MinionManager.GetMinions(_Player.ServerPosition, 
                ...
                switch (unit.Team)
                {
                    case GameObjectTeam.Neutral:
                        ...
                        if (unit.PassiveRooted() && Root.Item("jgaacount").GetValue<KeyBind>().Active &&
                            _Player.LSDistance(unit.ServerPosition) > 450)
                        {
                            return;
                        }
                        ...
                        break;
                    default:
                        if (unit.Team != _Player.Team && unit.Team != GameObjectTeam.Neutral)
                        {
        ...
        internal static void Flee()
        {
            ...
            var wallCheck = KL.GetFirstWallPoint(KL._Player.Position, Game.CursorPos);
            ...
            if (KL.CatForm() && KL.SpellTimer["Pounce"].IsReady() && wallCheck != null)
            {
                ...
                while (true)
                {
                    ...
                    if (KL._Player.GetPath(wallPositionOpposite).ToList().LSTo2D().LSPathLength() -
                        KL._Player.LSDistance(wallPositionOpposite) > 200)
                    {
                        if (KL._Player.LSDistance(wallPositionOpposite) < KL.Spells["Pounce"].Range - KL._Player.BoundingRadius / 2)
                        {
