    ...
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            skinitem.ValueChanged += (sender, eventArgs) =>
            {
                ...
                {
                    //ObjectManager.Player.SetSkin(ObjectManager.Player.CharData.BaseSkinName, 1);
                }
            };
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            {
                Render.Circle.DrawCircle(_Player.Position, Q.Range, Q.LSIsReady() ? Color.LawnGreen : Color.Red, 2);
            }

            if (Root.Item("drawr").GetValue<bool>())
            {
                Render.Circle.DrawCircle(_Player.Position, R.Range, R.LSIsReady() ? Color.LawnGreen : Color.Red, 2);
            }
        }
            ...
            if (Root.Item("supp").GetValue<bool>())
            {
                    ...
                    Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.LastHit)
                {
                    ...
                    if (minion != null && minion.IsMinion && minion.LSIsValidTarget())
                    {
                        ...
                        {
                            if (_Player.HasBuff("talentreaperdisplay"))
                            {
                                var b = _Player.GetBuff("talentreaperdisplay");
                                if (b.Count > 0)
                                {
                                    args.Process = true;
                                    return;
        ...
        private static void Interrupter2_OnInterruptableTarget(AIHeroClient sender, Interrupter2.InterruptableTargetEventArgs args)
        {
            ...
            {
                if (R.LSIsReady() && _Player.LSDistance(sender.ServerPosition) <= R.Range)
                {
                ...
                if (Q.LSIsReady() && _Player.LSDistance(sender.ServerPosition) <= Root.Item("maxq").GetValue<Slider>().Value)
                {
                    if (_Player.HealthPercent < Root.Item("grabhp").GetValue<Slider>().Value)
                    {
                        return;
                    }

                    if (!Root.Item("blq" + sender.ChampionName).GetValue<bool>() &&
                        _Player.LSDistance(sender.ServerPosition) > Root.Item("minq").GetValue<Slider>().Value)
                    {
                        Q.Cast(sender);
                    }
                }
        ...
        private static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (hero != null && hero.IsEnemy && Q.LSIsReady() && Root.Item("useqcombo").GetValue<bool>())
            {
                ...
                if (hero.LSIsValidTarget(Root.Item("maxq").GetValue<Slider>().Value) && hero.Health > Q.GetDamage(hero))
                {
                    ...
                    {
                        if (hero.LSDistance(_Player.ServerPosition) > Root.Item("minq").GetValue<Slider>().Value)
                        {
        ...
        {
            if (_Player.IsDead || !Orbwalking.CanMove(100))
            {
                return;
            }
            ...
            {
                _Player.SetSkin(_Player.CharData.BaseSkinName, Root.Item("skinid").GetValue<Slider>().Value);
            }
            ...
            foreach (var ene in HeroManager.Enemies.Where(x => x.LSIsValidTarget(Root.Item("maxq").GetValue<Slider>().Value)))
            {
                ...
                {
                    if (ene.LSDistance(_Player.ServerPosition) > Root.Item("minq").GetValue<Slider>().Value && Q.LSIsReady())
                    {
        ...
        private static void Flee(bool enable)
        {
            ...
            var ene = HeroManager.Enemies.FirstOrDefault(x => x.LSDistance(_Player.ServerPosition) <= E.Range + 200);
            if (E.LSIsReady() && ene.LSIsValidTarget())
            {
                E.Cast();
            }

            if (_Player.HasBuff("powerfist") && Orbwalking.InAutoAttackRange(ene))
            {
        ...
        private static void Combo(bool useq, bool usee, bool user)
        {
            ...
            if (useq && Q.LSIsReady())
            {
                ...
                if (!(_Player.HealthPercent < Root.Item("grabhp").GetValue<Slider>().Value))
                {
                    if (QT.LSIsValidTarget() && QT.LSDistance(_Player.ServerPosition) > Root.Item("minq").GetValue<Slider>().Value)
                    {
            ...
            if (usee && E.LSIsReady())
            {
                var ET =
                    HeroManager.Enemies.FirstOrDefault(
                        x => x.HasBuff("rocketgrab2") || x.LSDistance(_Player.ServerPosition) <= E.Range + 200);
