...
namespace BrianSharp
{
    ...
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            Loading.OnLoadingComplete += OnStart;
        }
        ...
        {
            var plugin = Type.GetType("BrianSharp.Plugin." + Player.Instance.ChampionName);
            if (plugin == null)
            {
                Helper.AddNotif(string.Format("[Brian Sharp] - {0}: Not Load !", Player.Instance.ChampionName), 3000);
                return;
            }
           ...
           // Orbwalk.Init(MainMenu);
            //_Orbwalker.Init(MainMenu);
            var zed = new _Orbwalker(MainMenu.SubMenu("Orb"));
            NewInstance(plugin);
            Helper.AddBool(
                MainMenu.SubMenu(Player.Instance.ChampionName + "_Plugin").SubMenu("Misc"),
                "UsePacket",
                "Use Packet To Cast");
            ...
            Helper.AddNotif(string.Format("[Brian Sharp] - {0}: Loaded !", Player.Instance.ChampionName), 3000);
        }
