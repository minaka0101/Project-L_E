...
namespace Azir
{
    class AzirWalker : Orbwalking.Orbwalker
    {
        ...
        {
            var d = soldierAttack ? Program.player.LSGetSpellDamage(target, SpellSlot.W) : Program.player.LSGetAutoAttackDamage(target);
                ...
                ActiveMode == Orbwalking.OrbwalkingMode.LastHit)
            {
                    ...
                    )
                {
                    ...
                    if(r != 0)
                    {
                        ...
                        var damage = (r == 1) ? Program.player.LSGetAutoAttackDamage(minion, true) : Program.player.LSGetSpellDamage(minion, SpellSlot.W);
