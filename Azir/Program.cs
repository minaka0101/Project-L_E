...
namespace Azir
{
    class Program
    {
        ...
        static void Interrupter2_OnInterruptableTarget(AIHeroClient sender, Interrupter2.InterruptableTargetEventArgs args)
        {
            ...
            if(Menu.SubMenu("Misc").Item("AutoEInterrupt").GetValue<bool>() && E.LSIsReady())
            {
                foreach (var soldier in SoldiersManager.AllSoldiers.Where(s => player.LSDistance(s, true) < E.RangeSqr))
                {
            ...
            if (Menu.SubMenu("R").Item("AutoRInterrupt").GetValue<bool>() && R.LSIsReady())
            {
                var dist = player.LSDistance(sender, true);
                ...
                if(dist < Math.Pow(Math.Sqrt(R.RangeSqr + Math.Pow(R.Width + sender.BoundingRadius, 2)) , 2))
                {
                    ...
                    var p = (sender.ServerPosition.LSTo2D() - player.ServerPosition.LSTo2D()).LSRotated(angle);
                    R.Cast(p);
                }
            }
        }
        ...
        static float GetComboDamage(Obj_AI_Base target)
        {
            ...
            damage += SoldiersManager.ActiveSoldiers.Count * player.LSGetSpellDamage(target, SpellSlot.W);
        ...
        static void LaneClear()
        {
            ...
            if (minions.Count > 0)
            {
                ...
                {
                    var p = player.Position.LSTo2D().LSExtend(minions[0].Position.LSTo2D(), W.Range);
                    W.Cast(p);
                    return;
                }

                if (useQ && Qline.LSIsReady() && (minions.Count >= 2 || minions[0].Team == GameObjectTeam.Neutral))
                {
        ...
        static void Harass()
        {
            ...
            {
                var p = player.Position.LSTo2D().LSExtend(harassTarget.Position.LSTo2D(), W.Range);
                if(Q.LSIsReady() || HeroManager.Enemies.Any(h => h.LSIsValidTarget(W.Range + 200)))
                {
                    W.Cast(p);
                }
                return;
        ...
        static void Combo()
        {
            ...
            {
                var p = player.LSDistance(qTarget, true) > W.RangeSqr ? player.Position.LSTo2D().LSExtend(qTarget.Position.LSTo2D(), W.Range) : qTarget.Position.LSTo2D();
                W.Cast(p);
            }

            if (useE && ((Utils.TickCount - _allinT) < 4000 || (HeroManager.Enemies.Count(e => e.LSIsValidTarget(1000)) <= 2 && GetComboDamage(qTarget) > qTarget.Health)) && E.LSIsReady())
            {
                foreach (var soldier in SoldiersManager.AllSoldiers2.Where(s => player.LSDistance(s, true) < E.RangeSqr))
                {
            ...
            if (GetComboDamage(qTarget) > qTarget.Health)
            {
                ...
                if (Menu.SubMenu("Combo").Item("UseIgnite").GetValue<bool>() && IgniteSlot != SpellSlot.Unknown && player.GetSpell(IgniteSlot).State == SpellState.Ready && player.LSDistance(qTarget, true) < 600 * 600)
                {
