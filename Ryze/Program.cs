...
namespace Ryze
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Utility.HpBarDamageIndicator.DamageToUnit += hero => (float)(Player.LSGetSpellDamage(hero, SpellSlot.Q) * 2 + Player.LSGetSpellDamage(hero, SpellSlot.W) + Player.LSGetSpellDamage(hero, SpellSlot.E));
        ...
        private static void OrbwalkingOnBeforeAttack(Orbwalking.BeforeAttackEventArgs args)
        {
            ...
            {
                args.Process = !(W.LSIsReady() || E.LSIsReady() || !W.IsInRange(args.Target));
            }
        }
        ...
        private static void Combo()
        {
            ...
            var castQ = coinChargeAmount == 0 || coinChargeAmount == 2 || !W.LSIsReady() && !E.LSIsReady() || qTarget != null && Q.IsKillable(qTarget) || target == null;
            if (castQ && Q.LSIsReady() && qTarget != null)
            {
            ...
            if (target != null)
            {
                if (!(W.LSIsReady() && W.GetRange(target) - 50 < Player.LSDistance(target)))
                {
        ...
        private static void Farm(bool laneClear)
        {
                ...
                foreach (var minion in allMinions)
                {
                    if (minion.LSIsValidTarget() &&
                        HealthPrediction.GetHealthPrediction(minion,
                            (int)(Player.LSDistance(minion) * 1000 / 1700)) <
            ...
            else if (useW && W.LSIsReady())
            {
                foreach (var minion in allMinions)
                {
                    if (minion.LSIsValidTarget(W.GetRange(minion)) &&
                        minion.Health < Player.LSGetSpellDamage(minion, SpellSlot.W))
                    {
                        W.CastOnUnit(minion);
                        return;
                    }
            ...
            else if (useE && E.LSIsReady())
            {
                foreach (var minion in allMinions)
                {
                    if (minion.LSIsValidTarget(E.GetRange(minion)) &&
                        HealthPrediction.GetHealthPrediction(minion,
                            (int)(Player.LSDistance(minion) * 1000 / 1000)) <
                        Player.LSGetSpellDamage(minion, SpellSlot.E) - 10)
                    {
                        E.CastOnUnit(minion);
                        return;
                    }
